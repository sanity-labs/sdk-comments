...
export function useCommentsState(
  serverComments: CommentDocument[] | undefined,
  {isPending = false}: UseCommentsStateOptions = {},
): CommentsState {
  const [optimisticAdds, setOptimisticAdds] = useState<CommentDocument[]>([])
  const [optimisticDeletes, setOptimisticDeletes] = useState<Set<string>>(new Set())
  const [optimisticEdits, setOptimisticEdits] = useState<Map<string, CommentOptimisticEdit>>(
    new Map(),
  )
  ...
  const editOptimisticComment = useCallback(
    (commentId: string, message: CommentMessage, lastEditedAt: string) => {
